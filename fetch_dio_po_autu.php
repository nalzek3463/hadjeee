...
if (isset($_GET['id_auta'])) {
    ...
    // Dohvati informacije o odabranom automobilu
    $carQuery = "SELECT * FROM auta WHERE id_auta = $carId";
    $carResult = mysqli_query($conn, $carQuery);
    $carData = mysqli_fetch_assoc($carResult);

    if ($carData) {
        $id_auta = $carData['id_auta'];
        

        // Izradite SQL upit za dobivanje dijelova koji odgovaraju marki i tipu automobila
        $sql = "SELECT * FROM dijelovi2 WHERE porijeklo = '$id_auta' ORDER BY id DESC";
        ...
        foreach ($products as $product) {
            $id = $product['id'];
            $marka = $product['marka1'];
            $tip = $product['tip1'];
            $proizvod = $product['dio'];
            $kataloskibroj = $product['katbroj'];
